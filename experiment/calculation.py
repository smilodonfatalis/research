...
def calc_q_train(df_train, q_trial_train, q_block_train, alpha, trial_size_train):
    ...
    for trial in range(trial_size_train):
        ...
        if idx_chosen==df_train[trial, 'idx_left']:
            idx_unchosen = df_train[trial, 'idx_right']
        else:
            idx_unchosen = df_train[trial, 'idx_left']
        
        # print(idx_chosen, idx_unchosen)
        q_delta = q_trial_train[idx_chosen] - q_trial_train[idx_unchosen]
        if df_train[trial, 'conf'] == 1:
            q_deltas_1.append(q_delta)
        elif df_train[trial, 'conf'] == 2:
            q_deltas_2.append(q_delta)
        elif df_train[trial, 'conf'] == 3:
            q_deltas_3.append(q_delta)
    ...
    q_delta_1 = np.array(q_deltas_1).mean()
    q_delta_2 = np.array(q_deltas_2).mean()
    q_delta_3 = np.array(q_deltas_3).mean()
        
    return q_delta_1, q_delta_2, q_delta_3
