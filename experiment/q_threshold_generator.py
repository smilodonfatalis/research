...
def main():
    ...
    for subj in range(1, subj_size+1):
        for pat in range(1, pat_size+1):
            ...
            q_delta_1, q_delta_2, q_delta_3 = calc.calc_q_train(df_slf, q_trial_slf, q_block_slf, alpha, seq_size_slf*block_size)
            q_deltas_1.append(q_delta_1)
            q_deltas_2.append(q_delta_2)
            q_deltas_3.append(q_delta_3)

            # log_like_softmax, p_softmax = calc.calc_log_like_test_ss_softmax(d
    ...
    q_deltas_1 = np.array(q_deltas_1)
    q_deltas_2 = np.array(q_deltas_2)
    q_deltas_3 = np.array(q_deltas_3)
    q_deltas_13 = (q_deltas_1 + q_deltas_3) / 2

    q_deltas_2 = q_deltas_2[~np.isnan(q_deltas_2)]
    q_deltas_3 = q_deltas_3[~np.isnan(q_deltas_3)]
    q_deltas_13 = q_deltas_13[~np.isnan(q_deltas_13)]

    # print(q_deltas_3)
    print(q_deltas_3.mean())

    # print(q_deltas_13)
    # print(q_deltas_13.mean())

    q_threshold = q_deltas_13.mean()
